...
class Blip2OPT(Blip2Base):
    ...
    def __init__(
        ...
        if llm_model == 'facebook/galactica-125m':
            ...
            # self.llm_model = OPTForCausalLM.from_pretrained(llm_model, torch_dtype=torch.float16)
            self.llm_model = OPTForCausalLM.from_pretrained(llm_model, torch_dtype=torch.bfloat16)
        self.llm_model.resize_token_embeddings(len(self.llm_tokenizer)) ## this will cause bug when full fine-tuning the opt model
    ...
    def forward_reaction(self, batch):
        ...
        if False:
            if self.llm_tune:
                react_embeds = self.llm_model.model.get_decoder().embed_tokens(reaction_tokens.input_ids) # shape = [B, max_len, D]
                notes_embeds = self.llm_model.model.get_decoder().embed_tokens(notes_tokens.input_ids)
            else:
                react_embeds = self.llm_model.model.decoder.embed_tokens(reaction_tokens.input_ids) # shape = [B, max_len, D]
                notes_embeds = self.llm_model.model.decoder.embed_tokens(notes_tokens.input_ids) # shape = [B, max_len, D]
        else:
            react_embeds = self.llm_model.get_input_embeddings()(reaction_tokens.input_ids)
            notes_embeds = self.llm_model.get_input_embeddings()(notes_tokens.input_ids)
        ...
        targets = notes_tokens.input_ids.masked_fill(
            notes_tokens.input_ids == self.llm_tokenizer.pad_token_id, -100
        )
        empty_targets = (
    ...
    def opt_qa(
        ...
        prompt_tokens = self.llm_tokenizer(prompts,
                                           truncation=False,
                                           padding='longest',
                                           add_special_tokens=True,
                                        #    max_length=self.args.max_len[],
    ...
    def probe_qformer(
        ...
        with self.maybe_autocast():
            ...
            ## 
            embed_func = self.llm_model.get_input_embeddings()
            ...
            knn_decode_strings = self.llm_tokenizer.batch_decode(topk_ids.flatten())
